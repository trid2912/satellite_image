...
class PADDataset(Dataset):
    ...
    def load_medians(self, path: Path, subpatch_id: int, start_bin: int) -> Tuple[np.ndarray, np.ndarray]:
        ...
        path_to_remove = path.parts[-3]
        modified_path = Path(*[part for part in path.parts if part != path_to_remove])
    ...
    def __getitem__(self, idx: int) -> dict:
        ...
        if self.saved_medians:
            ...
            path_to_remove = self.root_path_netcdf.parts[-1]
            modified_path = Path(*[part for part in self.root_path_netcdf.parts if part != path_to_remove])
            self.root_path_netcdf = modified_path
