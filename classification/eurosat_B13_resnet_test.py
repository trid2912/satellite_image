...
parser.add_argument('--data_dir', type=str, default='/mnt/disk1/tritd/datasets/EuroSATallBands')
parser.add_argument('--checkpoints_dir', type=str, default='./checkpoints/resnet_eurosat/')
parser.add_argument('--resume', type=str, default='')
parser.add_argument('--save_path', type=str, default='./checkpoints/resnet_eurosat/eurosat_s2_B12_100_no_pretrain_resnet50.pt')
...
def main():
    ...
    train_dataset = EurosatDataset(root=args.data_dir, transform = train_transforms, img_dir="train.csv")
    val_dataset = EurosatDataset(root=args.data_dir, transform = val_transforms, img_dir="validation.csv")
    test_dataset = EurosatDataset(root=args.data_dir, transform = val_transforms, img_dir="test.csv")
    ...
    def collate_fn(batch):
        max_size = [0, 0, 0]  # Assuming 3D tensors, adjust if different
        for item in batch:
            for i in range(3):  # Adjust loop if tensors have more dimensions
                max_size[i] = max(max_size[i], item[0].size(i))
        from torchvision.transforms.functional import pad
        # Pad inputs to the maximum size and collect labels
        inputs = [pad(item[0], [0, 0, max_size[2] - item[0].size(2), max_size[1] - item[0].size(1)]) for item in batch]
        labels = [torch.tensor(item[1]) for item in batch]
        
        # Stack padded inputs and convert labels to a tensor
    ...
    with torch.no_grad():
        ...
        for j, data_val in enumerate(test_loader, 0):
            ...
            score_val = torch.sigmoid(outputs_val).detach().cpu()
            average_precision_val = accuracy_score(labels_val.cpu(), torch.argmax(score_val,axis=1)) * 100.0   
            true_labels.extend(list(labels_val.cpu().numpy()))
            predictions.extend(list(torch.argmax(score_val,axis=1).cpu().numpy()))
            count_val += 1
            running_loss_val += loss_val.item()
            running_acc_val += average_precision_val        
