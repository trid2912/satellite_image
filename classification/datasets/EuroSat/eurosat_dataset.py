...
class EurosatDataset(Dataset):
    ...
    def __init__(self, root, split=None, bands='B13', transform=None, img_dir = None):
        self.root = Path(root)
        self.split = split
        ...
        if not img_dir:
            ...
            img_list = pd.read_csv(os.path.join(root,img_dir))["Filename"].tolist()
            for idx in range(len(img_list)):
                path = os.path.join(root, img_list[idx])
                self.samples.append(path)
                target = self.class_to_idx[Path(path).parts[-2]]
                self.targets.append(target)
...
if __name__ == '__main__':
    ...
    data_path = '/mnt/disk1/tritd/datasets/EuroSATallBands'
    batchsize = 4
    
    eurosat_dataset = EurosatDataset(root=data_path)
